...
class DoctorService {
    ...
    static async searchDoctors(query) {
        const { page = 1, limit = 10, search, specialty, hospital, availableDate, minFee, maxFee, minRating, sortBy = 'name', sortOrder = 'asc', } = query;
        // Build where clause
        const where = {
            isActive: true,
        };
    ...
    static async getDoctorById(doctorId) {
        ...
        if (!doctor) {
            throw new errorHandler_1.AppError('Doctor not found', 404);
        }
        return doctor;
    }
    /**
     ...
     */
    static async updateDoctor(doctorId, data) {
    ...
    static async deleteDoctor(doctorId) {
        ...
        const futureAppointments = await prisma_1.prisma.appointment.count({
            where: {
                doctorId,
                date: {
                    gte: new Date(),
        ...
        if (futureAppointments > 0) {
            throw new errorHandler_1.AppError('Cannot delete doctor with future appointments. Please cancel or reschedule existing appointments first.', 400);
        }
        const deletedDoctor = await prisma_1.prisma.doctor.update({
            where: { id: doctorId },
            data: { isActive: false },
        });
     ...
     */
    static async getAvailableSlots(doctorId, query) {
        const doctor = await prisma_1.prisma.doctor.findUnique({
        ...
        if (date) {
            ...
            const availableSlots = dateSlots.timeSlots.filter((slot) => slot.available && !bookedSlots.includes(slot.time));
            return {
                date,
        ...
        if (dateFrom && dateTo) {
            ...
            for (const dateSlot of availableDates) {
                ...
                if (slotDate >= fromDate && slotDate <= toDate) {
                    ...
                    const bookedSlots = existingAppointments.map(apt => apt.timeSlot);
                    // Filter available slots
                    const availableSlots = dateSlot.timeSlots.filter((slot) => slot.available && !bookedSlots.includes(slot.time));
                    slotsInRange.push({
                        date: dateSlot.date,
                        slots: availableSlots,
                    });
    ...
    static async getSpecialties() {
        const specialties = await prisma_1.prisma.doctor.findMany({
            where: { isActive: true },
    ...
    static async getHospitals() {
        const hospitals = await prisma_1.prisma.doctor.findMany({
            where: { isActive: true },
     ...
     */
    static async updateAvailability(doctorId, availableDates) {
        const doctor = await prisma_1.prisma.doctor.findUnique({
            where: { id: doctorId },
        });
        if (!doctor) {
